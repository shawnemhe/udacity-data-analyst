...
could easily be lost with a wider bin width. But transforming the data to log
brings out out even more, suggesting that the date is bi-modal. The 10 year rate
...
scale balances the emphasis placed on the rare occurrence of valuations at both
...
return was `r round(mean(cape$Momentum, na.rm = TRUE) * 100, 2) `%.
...
The distribution of CPIChange shows very high kurtosis, with a mean of about
...
positively skewed the longer the duration. This makes sense after seeing it.
The skew is the affect of compounding, but there is a lower limit due to prices
having never been below 0.
...
plt1 <- g + geom_histogram(aes(Fwd1yrReturns))
plt2 <- g + geom_histogram(aes(annualize(Fwd3yrReturns, 3))) +
                                    labs(x = "3 Year CAGR")
plt3 <- g + geom_histogram(aes(annualize(Fwd5yrReturns, 5))) +
                                    labs(x = "5 Year CAGR")
plt4 <- g + geom_histogram(aes(annualize(Fwd10yrReturns, 10))) +
                                    labs(x = "10 Year CAGR")
...
The oft referred to fat tails of the returns are present and a slight negative 
...
is a result of the negative skew and it's affect on compounding. Large losses
require even larger gains for recovery, causing actual CAGRs to be lower than 
the mean Fwd1yrReturns.

After seeing the transformed values I decided to store them easier access during
...
plt1 <- g + geom_point(aes((RealPrice), (RealEarnings)))
plt2 <- g + geom_point(aes((RealPrice), (RealDividend)))
plt3 <- g + geom_point(aes((RealDividend), (RealEarnings)))
...
plot_vs_returns(cape, cape$CAPE, "CAPE")
```
...
-0.37 and -0.4, in order of increasing forward looking periods.
...
g_cape <- ggplot(aes(x = cut(log10(CAPE), breaks = 5)), 
...
medians returns are more of an indication of what investors will receive during
a typical year.

### Momentum and Forward Returns

```{r echo=FALSE, message=FALSE, warning=FALSE, Momentum_Returns}
plot_vs_returns(cape, cape$Momentum, "Momentum")
```

There doesn't appear to be a strong correlation between Momentum and forward
returns. There is some over plotting, and it looks like there are outliers. It
might help to zoom in and add some transparency to the points.
...
learned from studying trend following  in the past: its main benefit is in
avoiding the worst periods. However, I was expecting at least some power in 
identifying higher future returns.
...
plot_vs_returns(cape, log10(cape$GS10), "Log(GS10)", alpha = 1/10)
```

There doesn't appear to be a correlation between interest rates and future 
returns.

```{r echo=FALSE, message=FALSE, warning=FALSE, GS10_CAPE}
ggplot(cape[-(1:12),]) + geom_point(aes(GS10, CAPE))+
  scale_x_log10() + scale_y_log10()
cor.test(cape$GS10, cape$CAPE)
```

It does look like there could be some negative correlation between GS10 rates 
and CAPE. But the `cor.test` function returns a low value indicating that it
is very weak.
...
a weak correlation of .36 between CPIChangeand Fwd10yrCAGR. It looks like 
...
to manage the inflation rate. It's possible that lagging one against the other
would reveal correlation. For example, it might show that the Fed raised rates
after high inflation, or that inflation had lowered after rising rates.
...
The strongest relationship was between CAPE and forward returns. The correlation
was stronger with multiyear forward looking periods than with 1 year.
...
see. Periods of falling growth in the stock market exhibit a stronger negative
...
The feature combination doesn't appear to have the same value over longer time
periods.
