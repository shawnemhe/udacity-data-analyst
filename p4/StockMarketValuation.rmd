...
100% and losing more than 50%. The boxplot makes these extreme occurances easy
...
medians returns are a better indication of what investors will receive during
a typical year.
...
ggplot(cape[-(1:12),]) + geom_point(aes(GS10, log10(CAPE)))
...
log_cut <- function(x, breaks = 5, digits = 0) {
...
cape_10yr <- ggplot(aes(x = log_cut(CAPE), y = Fwd10yrCAGR),
                    data = subset(cape, !is.na(CAPE))) +
  labs(x = "CAPE") + 
  scale_y_continuous(labels = scales::percent, limits = c(-.1, .2))

plt1 <- cape_10yr + geom_bar(stat = "summary", fun.y = mean) +
  ggtitle("Average Return")

plt2 <- cape_10yr + geom_boxplot() +
...
`r round(cape$CAPE[nrow(cape)])` as of `r cape$Date[nrow(cape)]`.
