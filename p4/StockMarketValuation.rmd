...
positively skewed the longer the duration. This is due to compounding effects,
and the lower limit on price (the stock market has never been priced below 0).
...
skew is visible. This is charactersitic of the risks investors face; extreme 
...
require even larger gains for recovery, causing actual CAGRs to be lower than the
mean Fwd1yrReturns.
...
The dataset contains 1769 months of data covering 19 features, one of which 
...
often used by governments to tame inflation and/or boost economic growth.
...
Log transformations were applied to prices, dividends and earnings to reduce postive
skew. They are also a natural transformation to apply because they convert the
expontential growth of continuous componding into an additive one. The CAPE ratio
is generated from those variables and benefited from the same transformation.
...
Last, I transformed the FwdXyrReturns to CAGRs so that they could be directly 
compared.
...
plt1 <- g + geom_point(aes(CAPE, Fwd1yrReturns))
plt2 <- g + geom_point(aes(CAPE, Fwd3yrReturns))
plt3 <- g + geom_point(aes(CAPE, Fwd10yrReturns))
plt4 <- g + geom_point(aes(CAPE, Fwd10yrReturns))
...
The cape ratio is negatively correlated to forward returns, but the effect is
...
plt1 <- g + geom_point(aes(CAPE, Fwd1yrReturns)) + scale_x_log10()
plt2 <- g + geom_point(aes(CAPE, annualize(Fwd3yrReturns, 3))) + scale_x_log10()
plt3 <- g + geom_point(aes(CAPE, annualize(Fwd10yrReturns, 5))) + scale_x_log10()
plt4 <- g + geom_point(aes(CAPE, annualize(Fwd10yrReturns, 10))) + scale_x_log10()
...
plt1 <- ggplot(cape, aes((CPIChange), (Fwd1yrReturns))) + geom_point() + geom_smooth(method = "lm") + scale_y_sqrt()
plt2 <- g + geom_point(aes(Momentum, Fwd3yrReturns))
plt3 <- g + geom_point(aes(Momentum, Fwd5yrReturns))
plt4 <- g + geom_point(aes(Momentum, Fwd10yrReturns))
...
```{r echo=FALSE, message=FALSE, warning=FALSE, LogMomentum_Returns}
plt1 <- ggplot(cape[-(1:12),]) + geom_boxplot(aes(Growth, Fwd1yrReturns))
plt2 <- ggplot(cape[-(1:12),]) + geom_boxplot(aes(Growth, Fwd3yrReturns))

print(plt1)
print(plt2)
```
Separating the data into Rising and Falling Growth periods does not appear to
provide any benefit at all in predicting higher returns. However, it does tighten
the distribution and redue the tails, a particular benefit in reducing risk. This 
is confirmation of what I have learned from studying trend following in the past:
the main benefit is not finding higher returns but in avoiding the worst.

### Dividends to Interest Rates
...
g + geom_jitter(aes(CAPE, Fwd3yrReturns, col = Growth)) + scale_x_log10()
```
