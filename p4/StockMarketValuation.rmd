...
plt2 <- g + geom_point(aes(CAPE, Fwd3yrCAGR))
plt3 <- g + geom_point(aes(CAPE, Fwd5yrCAGR))
plt4 <- g + geom_point(aes(CAPE, Fwd10yrCAGR))
...
plt1 <- plt1 + scale_x_log10() + labs(x = "Log10(CAPE)")
plt2 <- plt2 + scale_x_log10() + labs(x = "Log10(CAPE)")
plt3 <- plt3 + scale_x_log10() + labs(x = "Log10(CAPE)")
plt4 <- plt4 + scale_x_log10() + labs(x = "Log10(CAPE)")
...
plt1 <- g + geom_point(aes(Momentum, Fwd1yrReturns))
plt2 <- g + geom_point(aes(Momentum, Fwd3yrCAGR))
plt3 <- g + geom_point(aes(Momentum, Fwd5yrCAGR))
plt4 <- g + geom_point(aes(Momentum, Fwd10yrCAGR))
...
returns. There is some overplotting, and it looks like there are outliers. It
...
ggplot(aes(Growth, Fwd1yrReturns), data = cape[-(1:12),]) + geom_boxplot()
ggplot(aes(Growth, Fwd3yrCAGR), data = cape[-(1:12),]) + geom_boxplot()
```
...
tighten the distribution and reduce the tails, a particular benefit in reducing 
risk. This is confirmation of what I have learned from studying trend following 
in the past: the main benefit is not finding higher returns but in avoiding the 
worst.
